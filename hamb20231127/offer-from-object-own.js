document.addEventListener('DOMContentLoaded', () => {
  const weatherApp = {
    ...
    // Get the day number of the week from the date
    getDayOfWeek: function () {
      const date = new Date();
      const dayNumber = date.getDay();
      const daySelectInput = orderForm.querySelector('#daySelectInput');
      daySelectInput.selectedIndex = dayNumber;
      return dayNumber;
    },
    ...
    updateOutputs: function () {
      ...
      let celsiusDegree = this.getCelsiusDegree();
      celsiusOutput.textContent = `A várható hőmérséklet: ${celsiusDegree} °C (${this.celsiusToFahrenheit(
        celsiusDegree
      )} °F)`;
